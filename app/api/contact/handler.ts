...
import { getWaitlistProduct, serviceOptions, waitlistConfirmation, type ServiceOption } from "../../lib/product-catalog.ts";
...
function validatePayload(raw: unknown): { data?: ContactPayload; errors: ErrorMap } {
  ...
  const waitlistProduct = typeof raw.serviceInterest === "string"
    ? getWaitlistProduct(raw.serviceInterest.trim())
    : undefined;
  ...
  const companyName = readString(
    ...
    "Company name",
    errors,
    { required: !waitlistProduct, maxLength: 120 }
  );
  ...
  if (
    serviceInterest &&
    !serviceOptions.includes(serviceInterest as ServiceOption)
  ) {
  ...
  return {
    data: {
      ...
      // Preserve the verified CRM enum; record the exact product in the enquiry.
      serviceInterest: waitlistProduct ? "Not sure yet" : serviceInterest as HubSpotServiceOption,
      message: waitlistProduct
        ? `WAITLIST REGISTRATION\nProduct: ${waitlistProduct.name}\nStarting price: ${waitlistProduct.price}\nStatus: waitlist only; no access or purchase confirmed.\n${message ? `Note: ${message}` : "No additional note provided."}`
        : message,
      consent: true,
      ...(waitlistProduct ? { waitlist: true as const } : {}),
      pageUri,
      pageName,
    },
    errors,
...
export async function handleContactRequest(
  request: Request,
  submitLead: (input: HubSpotLeadInput) => ReturnType<typeof submitHubSpotLead> = submitHubSpotLead
) {
  ...
  return Response.json({
    ok: true,
    message: data.waitlist ? waitlistConfirmation : "We’ll respond within two business days.",
  });
}
